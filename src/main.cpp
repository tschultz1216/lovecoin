...
#if defined(NDEBUG)
# error "lovecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd4d8dcd5cbcf3cd96115255b814f32d6aa9da10a21cf5f03cd609d672e5b4552");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // lovecoin: starting difficulty is 1 / 2^12
...
{
    // lovecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // lovecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // lovecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // lovecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // lovecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // lovecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x25ad40364f91d333e14c754695529a26f59ff292c2061adaef52e56498da516c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 12/Jan/2018 President Trump Eschewss Hateful Rhetoric about 'Shithole countries'";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04bb40e5158337e90c03426c64fab19581bb98419093883aaa8227741179e8cc821ff9b697830965e8a43d164979487141af99cd2136bdb9eac5ccabf6b9115311") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515661102;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087361902;

        if (fTestNet)
        {
            block.nTime    = 1515661085;
            block.nNonce   = 385645548;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x593f3147f5a3716f01521bc33a0ea100ba8269b56073173f2ff7636619473ca5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc5, 0xb8, 0xda }; // lovecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("lovecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("lovecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static lovecoinMiner(CWallet *pwallet)
{
    printf("lovecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lovecoin-miner");
    ...
    try { loop {
        ...
        printf("Running lovecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("lovecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&lovecoinMiner, pwallet));
}
